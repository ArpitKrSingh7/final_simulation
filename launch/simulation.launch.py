...
def generate_launch_description():
    ...
    declare_gz_gui = DeclareLaunchArgument(
        ...
        default_value=PathJoinSubstitution(
            [FindPackageShare("sim_worlds_ercremote_pkg"), "config", "layout.config"]
        ),
        description="Simulation with Expected GUI Layout",
    )
    ...
    declare_components_config_path_arg = DeclareLaunchArgument(
        ...
        default_value=PathJoinSubstitution([
            FindPackageShare("sim_worlds_ercremote_pkg"), "config", "components.yaml"
        ]),
        description="Path to the components configuration.",
    )

    declare_start_location_arg = DeclareLaunchArgument(
        "start_location",
       default_value="1",
        description="Start location ID (1, 2, 3, 4, 5)",
       choices=["1", "2", "3", "4", "5"],
    )
    
    start_location = LaunchConfiguration("start_location")
    package_share_directory = FindPackageShare("sim_worlds_ercremote_pkg").find("sim_worlds_ercremote_pkg")
    locations_file_path = os.path.join(package_share_directory, "config", "start_positions.yaml")
    ...
    gz_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [FindPackageShare("sim_worlds_ercremote_pkg"), "launch", "final_world.launch.py"]
            )
        ),
        launch_arguments={
            "gz_gui": gz_gui,
